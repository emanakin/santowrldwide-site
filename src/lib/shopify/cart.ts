...
export async function createCart(items?: CartItem[]) {
  ...
  if (items && items.length > 0) {
    variables.input.lines = items.map((item) => ({
...
export async function addCartItems(cartId: string, items: CartItem[]) {
  const variables = {
    cartId,
...
export async function removeCartItems(cartId: string, lineIds: string[]) {
  const variables = {
    cartId,
    lineIds,
  };
...
export function shopifyCartToLocalCart(shopifyCart: any): {
  items: CartItem[];
  subtotal: number;
  tax: number;
  total: number;
